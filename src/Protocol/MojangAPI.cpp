...
void cMojangAPI::QueryNamesToUUIDs(AStringVector & a_NamesToQuery)
{
	while (!a_NamesToQuery.empty())
	{
		...
		auto [IsSuccessfull, Response] = cUrlClient::BlockingPost(m_NameToUUIDUrl, {}, std::move(RequestBody), MojangTrustedRootCAs::UrlClientOptions());
		if (!IsSuccessfull)
		{
			continue;
		}
...
void cMojangAPI::QueryUUIDToProfile(const cUUID & a_UUID)
{
	...
	auto [IsSuccessfull, Response] = cUrlClient::BlockingGet(Url, {}, {}, MojangTrustedRootCAs::UrlClientOptions());
	if (!IsSuccessfull)
	{
		return;
	}
